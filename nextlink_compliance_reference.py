...
def get_compliance_address_lists_block():
    ...
    # Remove existing lists first (compliance script does this)
    lines.append("rem [find list=EOIP-ALLOW]")
    lines.append("rem [find list=managerIP]")
    lines.append("rem [find list=BGP-ALLOW]")
    lines.append("rem [find list=SNMP]")
    lines.append("")
...
/radius
add address=142.147.112.2 secret=Nl22021234 service=dhcp src-address={loop_ip_clean} timeout=5s
add address=142.147.112.18 secret=Nl22021234 service=dhcp src-address={loop_ip_clean} timeout=5s
"""
