...
echo   1. Ollama AI Service (if installed)
echo   2. Flask Backend API (port 5000)
echo   3. HTML Frontend Server (port 8000)
echo.
echo ========================================
echo.
...
REM ===== Step 1: Start Ollama Service =====
...
if %ERRORLEVEL% EQU 0 (
    ...
    if exist "%OLLAMA_EXE%" (
        ...
        echo [1/3] Waiting for Ollama to initialize...
        timeout /t 3 /nobreak >nul
        
        REM Verify Ollama started
        curl -s http://localhost:11434/api/tags >nul 2>&1
        if %ERRORLEVEL% EQU 0 (
            echo [1/3] Ollama started successfully
        ) else (
            echo [1/3] NOTE: Ollama may still be starting (this is normal)
            echo [1/3]    It will be ready in a few seconds
        )
    ) else (
        echo [1/3] WARNING: Ollama not found at %OLLAMA_EXE%
        echo [1/3]    Install from: https://ollama.com/download
        echo [1/3]    Backend will work but AI features will be limited
    )
)
echo.

REM ===== Step 2: Start Flask Backend API =====
echo [2/3] Starting Flask Backend API...
...
if %ERRORLEVEL% EQU 0 (
    echo [2/3] WARNING: Backend API is already running on port 5000
    echo [2/3]    Skipping startup to avoid conflicts
) else (
    echo [2/3] Launching api_server.py...
    start "NOC Backend API" /min cmd /c "cd /d %~dp0 && python api_server.py"
    ...
    curl -s http://localhost:5000/api/health >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
        echo [2/3] Backend API started successfully
    ) else (
        echo [2/3] NOTE: Backend may still be starting (check window for errors)
    )
)
echo.

REM ===== Step 3: Start HTML Frontend Server =====
echo [3/3] Starting HTML Frontend Server...
REM Check if frontend is already running
curl -s http://localhost:8000/NOC-configMaker.html >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo [3/3] WARNING: Frontend server is already running on port 8000
    echo [3/3]    Skipping startup to avoid conflicts
) else (
    echo [3/3] Launching serve_html.py...
    start "NOC HTML Frontend" /min cmd /c "cd /d %~dp0 && python serve_html.py"
    echo [3/3] Waiting for frontend to initialize...
    timeout /t 2 /nobreak >nul
    echo [3/3] Frontend server started
)
echo.
