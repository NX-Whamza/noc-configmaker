...
Dynamic compliance: when GITLAB_COMPLIANCE_TOKEN + GITLAB_COMPLIANCE_PROJECT_ID are set,
get_all_compliance_blocks() will attempt to fetch live compliance data from the GitLab
netforge/compliance repository before falling back to the hardcoded blocks below.
"""

# GitLab dynamic compliance loader (optional — silently falls back when unavailable)
try:
...
def get_all_compliance_blocks(loopback_ip=None):
    ...
    Source priority:
      ...
      2. Hardcoded module constants below (always available)
    ...
    # 1. Try GitLab dynamic compliance first
    if _HAS_GITLAB_NCR and _get_gitlab_loader is not None:
        try:
            ...
            print(f"[COMPLIANCE-REF] GitLab loader failed, using hardcoded fallback: {_exc}")
